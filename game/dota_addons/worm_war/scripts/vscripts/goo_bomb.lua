		...
		if i ~= caster:GetTeamNumber() then
				 ...
				 if PlayerResource:IsValidPlayerID(playerID) then
					...
					ability:ApplyDataDrivenModifier(caster, hero, "modifier_goo_bomb", {goo_duration = 4.0, goo_slow = -70})
	   				local gooParticle = ParticleManager:CreateParticle("particles/units/heroes/hero_bristleback/bristleback_viscous_nasal_goo_debuff.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero)
	   				ParticleManager:SetParticleControl(gooParticle, 0, hero:GetAbsOrigin())
